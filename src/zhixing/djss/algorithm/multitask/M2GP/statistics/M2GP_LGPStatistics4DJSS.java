package zhixing.djss.algorithm.multitask.M2GP.statistics;
...
public class M2GP_LGPStatistics4DJSS extends LGPStatistics{
	@Override
	public void postEvaluationStatistics(final EvolutionState state) {
        ...
        if (output)
            ...
            for(int x = 0;x<subpops;x++){
            	if (bestOfGeneration[x] != null){
            		state.output.print("\t\t" + ((MultiObjectiveFitness)bestOfGeneration[x].fitness).fitness() + "\t" , statisticslog);
            		state.output.print("" + ((LGPIndividual)bestOfGeneration[x]).getTreesLength() + "\t" , statisticslog);
            		state.output.print("" + ((LGPIndividual)bestOfGeneration[x]).getEffTreesLength() + "\t" , statisticslog);
            		state.output.print("" + ((LGPIndividual)bestOfGeneration[x]).getEffTreesLength()/((LGPIndividual)bestOfGeneration[x]).getTreesLength() + "\t" , statisticslog);
            	}
//            	if (bestSoFar[x] != null){
//            		state.output.print("" + ((MultiObjectiveFitness)bestSoFar[x].fitness).fitness() + "\t" , statisticslog);
//            		state.output.print("" + bestSoFar[x].size() , statisticslog);
//            	}
...
//        }
        ...
        for(int x = 0;x<state.population.subpops.length; x++) {
        	for(int ti = 0; ti< 50; ti++) {
		        ...
		        List<ec.Fitness> fitnesses = new ArrayList();
		        fitnesses.add(ind.fitness);
		        ((M2GP_MT_EvaluationModel4Ind)evaluationModel).evaluate(fitnesses, (CpxGPInterface4DJSS)ind, x, state);
