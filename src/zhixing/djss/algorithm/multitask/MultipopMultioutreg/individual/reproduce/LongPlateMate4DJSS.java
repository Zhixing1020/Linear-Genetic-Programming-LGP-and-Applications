...
import zhixing.cpxInd.algorithm.multitask.MultipopMultioutreg.individual.reproduce.LongPlateMate;
...
public class LongPlateMate4DJSS extends LongPlateMate {
...
//	}
...

    {
        ...
        {        	
        	...
        	for(int i = 0; i<((MFEA_Evaluator4DJSS)state.evaluator).getNumTasks(); i++){
        	...
        	for(int t = 0;t<numTasks;t++){
        		double tmp_rnk;
        		if(((LGPIndividual_MPMO4DJSS)parents[t]).scalarRank_v.size()==numTasks){
        			tmp_rnk = ((LGPIndividual_MPMO4DJSS)parents[t]).scalarRank_v.get(t);
        		}
        		else{
        			tmp_rnk = ((LGPIndividual_MPMO4DJSS)parents[t]).scalarRank_v.get(0);
        		}
        		if(tmp_rnk<minRnk
        	...
        	for(int t = 0;t<numTasks;t++){
        		if(((LGPIndividual_MPMO4DJSS)parents[t]).getOutputRegisters().length > 1){
...
//        	int numTasks = ((MFEA_Evaluator4DJSS)state.evaluator).getNumTasks();
//        	
//        	LGPIndividual_MPMO4DJSS par[] = new LGPIndividual_MPMO4DJSS[numTasks];
...
//        		par[i] = (LGPIndividual_MPMO4DJSS) ((LGPIndividual)parents[i]).lightClone();
...
//    				if(((LGPIndividual_MPMO4DJSS)j1).getEffTreesLength(taskid) <= length){
