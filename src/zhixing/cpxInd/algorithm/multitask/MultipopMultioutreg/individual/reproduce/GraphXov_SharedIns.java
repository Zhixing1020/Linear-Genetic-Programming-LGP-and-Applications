...
public class GraphXov_SharedIns extends GraphCrossover{
	public static final String SHARED_GRAPH_CROSSOVER_MULTITASK = "shared_graph_cross_MT";
...
//	public double shared_ins_rate;
	...
	public void setup(final EvolutionState state, final Parameter base){
		...
		Parameter def = LGPDefaults.base().push(SHARED_GRAPH_CROSSOVER_MULTITASK);
...
//		}
		...
		//tossSecondParent = true; //because in MPMO, individuals from different sub populations have different number of objectives, they cannot be exchanged directly
...
//		return LGPDefaults.base().push(SHARED_GRAPH_CROSSOVER_MULTITASK);
//	}
...
//        }
        ...
        for(int q=start, parnt = 0;q<n+start; /* no increment */)  // keep on going until we're filled up
        	...
        	tarSF = state.random[0].nextInt(((MFEA_Evaluator)state.evaluator).getNumTasks());
        	...
        	if(subpopulation > 0){
        		...
        		((TournamentSel_SharedIns)sources[0]).produce(1,1,0,subpopulation,parents,state,thread, tarSF);
        	}
        	if(trial > 0){
        		sources[1].produce(1,1,1,trial,parents,state,thread);
        	}
        	else{
        		((TournamentSel_SharedIns)sources[1]).produce(1,1,1,trial,parents,state,thread, tarSF);
        	}
...
//        	for(int s : j1.getOutputRegisters()){
//        		outRegs1.add(s);
//        	}
        	Set<Integer> outRegs2 = new HashSet<>();
//        	for(int s : j2.getOutputRegisters()){
//        		outRegs2.add(s);
//        	}
        	
        	int mainoutput1, mainoutput2;
        	if(j1.getOutputRegisters().length > 1 && tarSF < j1.getOutputRegisters().length){
        		outRegs1.add(j1.getOutputRegisters()[tarSF]);
        		mainoutput1 = j1.getOutputRegisters()[tarSF];
        		int outi = getSecondSkilledTask((LGPIndividual_MPMO)j1, tarSF);
        		outRegs1.add(j1.getOutputRegisters()[outi]);
        	}
        	else{ //there is only one output register for the parent
        		outRegs1.add(j1.getOutputRegisters()[0]);
        		mainoutput1 = j1.getOutputRegisters()[0];
        	}
        	if(j2.getOutputRegisters().length > 1 && tarSF < j2.getOutputRegisters().length){
        		outRegs2.add(j2.getOutputRegisters()[tarSF]);
        		mainoutput2 = j2.getOutputRegisters()[tarSF];
        		int outi = getSecondSkilledTask((LGPIndividual_MPMO)j2, tarSF);
        		outRegs2.add(j2.getOutputRegisters()[outi]);
        	}
        	else{
        		outRegs2.add(j2.getOutputRegisters()[0]);
        		mainoutput2 = j2.getOutputRegisters()[0];
        	}
...
//        	}
        		...
        		// the intersection of output registers is larger than 1, which means we can select the crossover on common building blocks
        		//identify the intersection graph of output registers, and select a segment as candidate swapping sub graph
        		
        		ArrayList<Integer> IntersecGraph1 = getIntersecSubGraph(j1, outRegs1, mainoutput1);
        		ArrayList<Integer> IntersecGraph2 = getIntersecSubGraph(j2, outRegs2, mainoutput2);
    	        ...
    	        for(int t = 0;t<numTries;t++){
    	            ...
    	            Iterator<Integer> it = IntersecGraph1.iterator();
    	            while(it.hasNext()){
    	        		int ind = it.next();
    	        		if(ind <= begin1){
    	        			cand1.add(ind);
...
//        return 0;
	...
	protected ArrayList<Integer> getIntersecSubGraph(LGPIndividual ind, Set<Integer> tar, int maintarget){
		ArrayList<Integer> res=null, res_tmp=null, tmp=null;
		
		res = ind.getSubGraph(ind.getTreesLength()-1, new Integer[]{maintarget});
		
		for(int output : tar){
			tmp = ind.getSubGraph(ind.getTreesLength()-1, new Integer[]{output});
			if(res_tmp == null){
	...
	protected int getSecondSkilledTask(LGPIndividual_MPMO ind, int tarSF){
