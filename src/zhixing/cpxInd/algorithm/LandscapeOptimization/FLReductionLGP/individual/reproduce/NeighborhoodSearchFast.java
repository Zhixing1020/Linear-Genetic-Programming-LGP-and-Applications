...
public class NeighborhoodSearchFast extends NeighborhoodSearch{
...
//			
//		}
		inds[start] = all_in_one_produce(subpopulation, inds, state, thread, parents);
...
//		return pgSource;
		...
		for(int i = 0; i<newind.getTreesLength(); i++) {
			if(direction.get(i) != 0) {
				...
				GPTreeStruct newInstr = ((IndexList4LGP_FLR)indexlist).getRandSymbolByIndex(newsym, state, thread, subpopulation);
	...
	@Override
	protected boolean maintainPhenotype(EvolutionState state, int thread, LGPIndividual oldind, LGPIndividual newind,
			GenoVector newgv) {
		System.err.print("NeighborhoodSearchFast does not implement  maintainPhenotype() method");
		System.exit(1);
		return false;
	}
