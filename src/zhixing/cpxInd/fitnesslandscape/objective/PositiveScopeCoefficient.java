...
import zhixing.cpxInd.fitnesslandscape.objective.NegativeScopeCoefficient.FitnessComparotor;
import zhixing.cpxInd.individual.CpxGPIndividual;

public class PositiveScopeCoefficient extends FLMetrics{
	public static final String PSC = "posScoCoef";
	...
	public void setup(final EvolutionState state, final Parameter base) {
		...
		Parameter def = new Parameter(PSC);
		
		numBins = state.parameters.getIntWithDefault(base.push(P_NUMBINS), base.push(P_NUMBINS), 20);
		if(numBins < 2 ) {
...
//				if(indi.fitness.fitness() >= minfit + bini*binStep && indi.fitness.fitness() < minfit + (bini+1)*binStep)
...
//		}
		for(int i = 0; i<Mcal.size()-1; i++) {
			if(Mcal.get(i + 1) - Mcal.get(i) != 0) {
				nsc += Math.max(0, (Ncal.get(i+1) - Ncal.get(i)) / (Mcal.get(i+1) - Mcal.get(i)));
			}
		}
