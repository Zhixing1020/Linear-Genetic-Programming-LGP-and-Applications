...
public class Args4TanhRegFunc extends Args4Function_SRMT{

	public final static String P_NAME = "TanhRF_Argument";
...
//        	setValue(j, (1-step)*getValue(j) + step*W[j]);
	...
	protected double [] calculateGradient(double [][] X, double [] Y, double [] W) {
		...
		for(int i = 0; i<n_instance; i++) {
			...
			//partial W0
			double pw0 = 2*(tmpW[1] * Math.tanh(tmpW[2]*X[i][0] + tmpW[3]) + tmpW[0]  + X[i][0] - Y[i]);
			
			double pw1 = pw0*Math.tanh(tmpW[2]*X[i][0] + tmpW[3]);
			
			double pw2 = pw0*tmpW[1]*( 1 - Math.pow(Math.tanh(tmpW[2]*X[i][0] + tmpW[3]), 2) )*X[i][0];
			
			double pw3 = pw0*tmpW[1]*( 1 - Math.pow(Math.tanh(tmpW[2]*X[i][0] + tmpW[3]), 2) );
...
//			}
	...
	protected double getLoss(double [][] X, double [] Y, double [] W) {
		...
		for(int i = 0; i<X.length; i++) {
			...
			Loss += Math.pow((W[0] + W[1]*Math.tanh(W[2]*X[i][0]+W[3]) + X[i][0] - Y[i]), 2);
		}
