...
public class Args4ExpoRegFunc extends Args4Function_SRMT{

	public final static String P_NAME = "ExpoRF_Argument";
...
//	}
	...
	protected double[] calculateGradient(double[][] X, double[] Y, double[] W) {
		...
		for(int i = 0; i<n_instance; i++) {
			...
			//partial W
			double pw0 = 2*(Math.pow(tmpW[1]*tmpW[1]+1, X[i][0]) + tmpW[0] + X[i][0] - Y[i]);
			double pw1 = pw0 * Math.pow(tmpW[1]*tmpW[1]+1, X[i][0]-1) * 2 * X[i][0]*tmpW[1];
	...
	protected double getLoss(double[][] X, double[] Y, double[] W) {
		...
		for(int i = 0; i<X.length; i++) {
			...
			Loss += Math.pow(( W[0] + Math.pow(W[1]*W[1]+1, X[i][0]) + X[i][0] - Y[i]), 2);
		}
