package zhixing.symbreg_multitarget.util;
...
import zhixing.symbreg_multitarget.algorithm.entity.individual.primitive.Element_LR;
...
import zhixing.symbreg_multitarget.algorithm.entity.individual.primitive.TanhRegFunc;
...
public class LispParser4SRMT{
	...
	public static LGPIndividual4SRMT parseSRMTLGPRule(String expression, int numRegs, int maxIterations) {
        GPTree tree = null;
        String line;
		LGPIndividual4SRMT rule = new LGPIndividual4SRMT();
	...
	private static GPNode parseNode(String expression) {
        ...
        if (expression.charAt(0) == '(') {
            ...
            else if(func.startsWith("TanhRF_entity")) {
            	node = new TanhRegFunc();
            	int nextLeftBracketIdx = expression.indexOf('[');
            	int nextRightBracketIdx = expression.indexOf(']');
            	String args_str = expression.substring(nextLeftBracketIdx, nextRightBracketIdx+1);
            	((TanhRegFunc)node).setFromString(args_str);
            	int num_child = node.expectedChildren();
            	node.children = new GPNode[num_child];
            	for(int c = 0; c<num_child; c++) {
            		node.children[c] = parseNode(args.get(c));
            ...
            else if(expression.startsWith("Element_LR")) {
            	node = new Element_LR();
            	((Element_LR)node).setFromString(expression);
            }
	...
	public static void main(String[] args) {
        ...
        for (int run =  12; run < numRuns; run++) {
            ...
            LGPIndividual4SRMT rule = LispParser4SRMT.parseSRMTLGPRule(bestExpression, numRegs, maxIterations);
