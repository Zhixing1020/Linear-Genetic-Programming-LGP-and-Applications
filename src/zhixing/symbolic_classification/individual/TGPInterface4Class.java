...
public interface TGPInterface4Class extends CpxGPInterface4Class{

	
	default Double[] execute_outs(EvolutionState state, int thread, GPData input, ADFStack stack, GPIndividual individual, Problem problem) {
	
	    Double [] res = new Double[individual.getTrees().length];
	    for(int d = 0; d<individual.getTrees().length; d++ ) {
	    	DoubleData tmp = new DoubleData();
	    	individual.getTrees()[d].child.eval(state, thread, tmp, stack, (GPIndividual) this, problem);
			res [d] = tmp.value;
		}
	    return res;
...
//	default Double[] predict_label(EvolutionState state, int thread, GPData input, ADFStack stack, GPIndividual individual, Problem problem, Double [] outs) {
//		//returns the predicting label(s) for an instance
//		
////		LGPIndividual4Class ind = (LGPIndividual4Class) individual;
////		
...
////		}
//		
////		final int classnum = ((GPClassification) problem).getClassNum();
//		
...
////		}
//		
//		//compare the probability in class output registers, output the labels with the maximum probability
////		double [] program_out = new double [classnum];
////		for(int c = 0;c<classnum;c++) {
////			program_out[c] = outs[c]; //by default, the actual outputs after wrapping are in the first classnum registers. 
////		}
////		
////		((LGPIndividual4Class)ind).setupClassLabels((GPClassification) problem);
//		double label = ((GPClassification) problem).getClassLabelFromOutput(outs);
//		
...
//	}
	...
	default void resetRegisters(final Problem problem, double val, final CpxGPIndividual ind){
		...
		System.err.print("resetRegisters in TGPInterface4Class is a null method\n");
	}
...
//		return ind.getRegisters()[ind.getOutputRegister()[0]];
		...
		System.err.print("individuals for classification problems should not use this function to execute programs, try to use \"execute_outs(...)\" \n");		
		System.exit(1);
...
//		ind.setInitReg_values( null /*new AttributeGPNode [ind.getNumRegs()]*/);
		...
		System.err.print("prepareExecution in TGPInterface4Class is a null method\n");
	}
...
//		return result;
		...
		System.err.print("makeGraphvizRule in TGPInterface4Class is a null method\n");
		return null;
	}
