...
public class EvolutionState implements Singleton
    ...
    public final static String P_NODEEVALUATIONS = "nodeevaluations";
    ...
    */
    public void setup(final EvolutionState state, final Parameter base)
        ...
        p = new Parameter(P_NODEEVALUATIONS);
        if (parameters.exists(p, null)) {
        	numNodeEva = parameters.getDouble(p, null, 1);  // 0 would be UDEFINED    
        	
        	if (numNodeEva <= 0)
                output.fatal("If defined, the number of node evaluations must be an integer >= 1.", p, null);
        }
