...
public class CrossoverPipeline extends GPBreedingPipeline
    ...
    public int produce(final int min, 
        ...
        for(int q=start;q<n+start; /* no increment */)  // keep on going until we're filled up
         ...
         // at this point, parents[] contains our two selected individuals
            GPIndividual[] parnts = new GPIndividual[2];
        	for(int ind = 0 ; ind < parnts.length; ind++){
        		parnts[ind] = (GPIndividual) this.parents[ind]; 
  ...
  //===================LGP steady-state EA, zhixing, 2023.2.27 ===============
    public int produce(final int min, 
	        final int max, 
	        final int start,
	        final int subpopulation,
        ...
        if (!state.random[thread].nextBoolean(likelihood))
    	 ...
    	 for(int q=start, parnt = 0;q<n+start; /* no increment */) {
    		// are our tree values valid?
    	        if (tree1!=TREE_UNFIXED && (tree1<0 || tree1 >= ((GPIndividual)parents[0]).getTreesLength()))//========zhixing, 2021.3.26
    	            // uh oh
    	        ...
    	        if (tree2!=TREE_UNFIXED && (tree2<0 || tree2 >= ((GPIndividual) parents[1]).getTreesLength()))//========zhixing, 2021.3.26
    	            // uh oh
    	        ...
    	        if (tree1==TREE_UNFIXED || tree2==TREE_UNFIXED) 
    	            ...
    	            do
    	                ...
    	                if (tree1==TREE_UNFIXED) 
    	                    if (((GPIndividual) parents[0]).getTreesLength() > 1) //========zhixing, 2021.3.26
    	                        t1 = state.random[thread].nextInt(((GPIndividual) parents[0]).getTreesLength());//========zhixing, 2021.3.26
    	                    else t1 = 0;
    	                else t1 = tree1;

    	                if (tree2==TREE_UNFIXED) 
    	                    if (((GPIndividual) parents[1]).getTreesLength()>1)//========zhixing, 2021.3.26
    	                        t2 = state.random[thread].nextInt(((GPIndividual) parents[1]).getTreesLength());//========zhixing, 2021.3.26
    	                    else t2 = 0;
    	                else t2 = tree2;
    	                } while (((GPIndividual) parents[0]).getTree(t1).constraints(initializer) != ((GPIndividual) parents[1]).getTree(t2).constraints(initializer));//========zhixing, 2021.3.26
    	            }
    	        else
    	            {
    	            t1 = tree1;
    	            ...
    	            if (((GPIndividual) parents[0]).getTree(t1).constraints(initializer) //========zhixing, 2021.3.26
    	                != ((GPIndividual) parents[1]).getTree(t2).constraints(initializer)) // uh oh   //========zhixing, 2021.3.26
    	        ...
    	        for(int x=0;x<numTries;x++)
    	            ...
    	            p1 = nodeselect1.pickNode(state,subpopulation,thread,(GPIndividual) parents[0],((GPIndividual) parents[0]).getTree(t1));//========zhixing, 2021.3.26
    	            
    	            // pick a node in individual 2
    	            p2 = nodeselect2.pickNode(state,subpopulation,thread,(GPIndividual) parents[1],((GPIndividual) parents[1]).getTree(t2)); //========zhixing, 2021.3.26
    	        ...
    	        for(int x=0;x<j1.getTreesLength();x++)
    	            ...
    	            if (x==t1 && res1)  // we've got a tree with a kicking cross position!
    	                { 
    	                tree = (GPTree)(((GPIndividual) parents[0]).getTree(x).lightClone());
    	                tree.owner = j1;
    	                tree.child = ((GPIndividual) parents[0]).getTree(x).child.cloneReplacing(p2,p1); 
    	                ...
    	                tree = (GPTree)(((GPIndividual) parents[0]).getTree(x).lightClone());
    	                tree.owner = j1;
    	                tree.child = (GPNode)(((GPIndividual) parents[0]).getTree(x).child.clone());
    	                tree.child.parent = tree;
    	                tree.child.argposition = 0;
    	                j1.setTree(x, tree);
    	                }
    	        ...
    	        if (n-(q-start)>=2 && !tossSecondParent) 
    	            for(int x=0;x<j2.getTreesLength();x++)
    	                ...
    	                if (x==t2 && res2)  // we've got a tree with a kicking cross position!
    	                    { 
    	                    tree = (GPTree)(((GPIndividual) parents[1]).getTree(x).lightClone());           
    	                    tree.owner = j2;
    	                    tree.child = ((GPIndividual) parents[1]).getTree(x).child.cloneReplacing(p1,p2); 
    	                    tree.child.parent = tree;
    	                    tree.child.argposition = 0;
    	                    j2.setTree(x, tree);
    	                    j2.evaluated = false; 
    	                    ...
    	                    tree = (GPTree)(((GPIndividual) parents[1]).getTree(x).lightClone());           
    	                    tree.owner = j2;
    	                    tree.child = (GPNode)(((GPIndividual) parents[1]).getTree(x).child.clone());
    	                    tree.child.parent = tree;
    	                    tree.child.argposition = 0;
    	                    j2.setTree(x, tree);
    	                    }
