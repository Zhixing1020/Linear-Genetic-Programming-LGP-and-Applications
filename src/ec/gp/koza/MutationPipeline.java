...
public class MutationPipeline extends GPBreedingPipeline
  ...
  //===================LGP steady-state EA, zhixing, 2023.2.27 ===============
    public Individual produce(final int subpopulation,
    	...
    	if (tree!=TREE_UNFIXED && (tree<0 || tree >= ((GPIndividual) i).getTreesLength()))//========zhixing, 2021.3.26
            // uh oh
        ...
        if (tree==TREE_UNFIXED)
            if (((GPIndividual) i).getTreesLength()>1) t = state.random[thread].nextInt(((GPIndividual) i).getTreesLength());//========zhixing, 2021.3.26
            else t = 0;
        else t = tree;
        ...
        for(int x=0;x<numTries;x++)
            ...
            p1 = nodeselect.pickNode(state,subpopulation,thread,(GPIndividual) i,((GPIndividual) i).getTree(t));//========zhixing, 2021.3.26
            ...
            p2 = builder.newRootedTree(state,
                ...
                thread,
                p1.parent,
                ((GPIndividual) i).getTree(t).constraints(initializer).functionset,
                p1.argposition,
        ...
        if (sources[0] instanceof BreedingPipeline)
            ...
            for(int x=0;x<j.getTreesLength();x++)
                ...
                if (x==t && res)  // we've got a tree with a kicking cross position!
                    {
                    tree = (GPTree)(((GPIndividual) i).getTree(x).lightClone());
                    tree.owner = j;
                    tree.child = ((GPIndividual) i).getTree(x).child.cloneReplacingNoSubclone(p2,p1);
                    ...
                    tree = (GPTree)(((GPIndividual) i).getTree(x).lightClone());
                    tree.owner = j;
                    tree.child = (GPNode)(((GPIndividual) i).getTree(x).child.clone());
                    tree.child.parent = tree;
                    tree.child.argposition = 0;         
                    j.setTree(x, tree);
                    }
